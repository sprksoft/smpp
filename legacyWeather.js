async function updateWeatherDivs(weatherData, isBig, timeDifferenceMins) {
  ...
  const windSpeedKMH = Math.round(Number(wind.speed) * 3.6);
  ...
  try {
    ...
    weatherDiv.querySelector(".weather-temperature").innerText = temp + "°C";
    ...
    weatherDiv.querySelector(".weather-lastupdate").innerText =
      timeDifferenceMins == 0 ? "Now" : timeDifferenceMins + " min ago";
    weatherDiv.querySelector(".weather-feelslike") &&
      (weatherDiv.querySelector(".weather-feelslike").innerText =
    ...
    switch (mainWeather) {
      ...
      default:
        console.warn("Main weather not recognized:", mainWeather);
    }
...
async function createWeatherApp(location, isBig) {
  ...
  let timeDifferenceMins = Math.round(
    Math.abs(lastUpdateDate - new Date()) / 1000 / 60
  );
  let weatherData = weatherAppData.weatherData;
  if (
    timeDifferenceMins > 10 ||
    location != weatherAppData.lastLocation ||
    wasMigrated ||
    !weatherData
  ) {
    weatherData = await browser.runtime.sendMessage({
