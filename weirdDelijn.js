...
async function displayLijnenBasedOnHalte(entiteitnummer, haltenummer) {
  ...
  if (
    !delijnData ||
    !delijnData.halteDoorkomsten ||
    delijnData.halteDoorkomsten.length === 0
  ) {
    displayDelijnInfo(
      "Er zijn momenteel geen bussen beschikbaar voor deze stop."
    );
    return;
  }
...
async function createHalteDoorkomst(doorkomst) {
  ...
  if (predictionStatuses.includes("GESCHRAPT")) {
    ...
    arrivalTimeDeviation = ETA
      ? Math.round((ETA - originalETA) / 1000 / 60)
      : "No data";
    timeUntilDeparture = calculateTimeUntilDepartureInMins(ETA);
...
async function updateLijnCardWithApiData(
  ...
  try {
    ...
    const backgroundColor = getHexByCode(
      individualLijnDataColors.achtergrond.code
    );
    const borderColor = getHexByCode(
...
function addDelijnAttest() {
  const delijnBottomContainer = document.getElementById(
    "delijnBottomContainer"
  );
...
async function showHalteOptions() {
  ...
  const delijnHaltesData = await browser.runtime.sendMessage({
    action: "fetchDelijnData",
    url: `https://api.delijn.be/DLZoekOpenData/v1/zoek/haltes/${searchQuery}?maxAantalHits=${delijnSearchResultLimit}`,
  });
  ...
  delijnHaltesData.haltes.forEach((halte) => {
    halteSleutels =
      halteSleutels + halte.entiteitnummer + "_" + halte.haltenummer + "_";
  });
  const delijnHaltesLijnrichtingenData = await browser.runtime.sendMessage({
    action: "fetchDelijnData",
    url: `https://api.delijn.be/DLKernOpenData/api/v1/haltes/lijst/${halteSleutels}/lijnrichtingen`,
  });
  ...
  try {
    removeDelijnInfo();
    let i = 0;
    for (const halte of delijnHaltesLijnrichtingenData.halteLijnrichtingen.slice(
      delijnSearchResultLimit - 5
    )) {
...
async function createHalteOption(delijnHalteData, i) {
  ...
  const richtingen =
    "Naar: " + gethalteDirections(delijnHalteData.lijnrichtingen);
  ...
  let lijnenArray = [];
  delijnHalteData.lijnrichtingen
    .slice(0, 5)
    .forEach(async function (lijnrichting) {
      ...
      let individualLijnData = await browser.runtime.sendMessage({
        action: "fetchDelijnData",
        url: `https://api.delijn.be/DLKernOpenData/api/v1/lijnen/${lijnrichting.entiteitnummer}/${lijnrichting.lijnnummer}`,
      });
      ...
      let individualLijnDataColors = await browser.runtime.sendMessage({
        action: "fetchDelijnData",
        url: `https://api.delijn.be/DLKernOpenData/api/v1/lijnen/${lijnrichting.entiteitnummer}/${lijnrichting.lijnnummer}/lijnkleuren`,
      });
      let backgroundColor = getHexByCode(
        individualLijnDataColors.achtergrond.code
      );
      let borderColor = getHexByCode(
        individualLijnDataColors.achtergrondRand.code
      );
  ...
  halteLijnCard.addEventListener("click", async function (e) {
    ...
    await displayLijnenBasedOnHalte(
      this.dataset.entiteitnummer,
      this.dataset.haltenummer
    );
  });
...
async function migrateDelijnData() {
  await browser.runtime.sendMessage({
    action: "setDelijnAppData",
    data: JSON.parse(localStorage.getItem("lijnData")),
  });
...
class DelijnWidget extends WidgetBase {
  ...
  createDelijnInfo() {
    ...
    document
      .getElementById("delijnBottomContainer")
      .appendChild(delijnInfoContainer);
  }

  hideDelijnInfo() {
    let delijnInfoContainer = document.getElementById("delijnInfoContainer");
    if (!delijnInfoContainer) {
      createDelijnInfo();
      delijnInfoContainer = document.getElementById("delijnInfoContainer");
    }
  ...
  async createContent() {
    this.delijnContainer = document.createElement("div");
    ...
    this.delijnContainer.appendChild(delijnTopContainer);
    this.delijnContainer.appendChild(delijnBottomContainer);
    this.displayDelijnInfo("Loading...");
