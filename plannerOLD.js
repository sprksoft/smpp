class PlannerWidget extends WidgetBase {
  constructor() {
    super();
    this.pixelsPerMinute = 1.46;
    this.daysAddedOnTop = 0;
    this.plannerContainer = null;
  }

  async fetchPlannerData(date, user) {
    try {
  ...
  calculateElementHeight(startTime, endTime) {
    const durationInSeconds = (endTime - startTime) / 1000;
    const durationInMinutes = durationInSeconds / 60;
    return durationInMinutes * this.pixelsPerMinute;
  }

  fancyfyTime(inputTime) {
  ...
  async getDateInCorrectFormat(isFancyFormat, addend) {
  ...
  createPlannerElement(
    ...
    timeElement.textContent = this.fancyfyTime(
    ...
    const height = this.calculateElementHeight(dateTimeFrom, dateTimeTo);
    const top = this.calculateElementHeight(beginTime, dateTimeFrom);
    ...
    // Add margin if not the last element
  ...
  async updatePlanner(addend) {
    if (!this.plannerContainer) {
      this.plannerContainer = this.createPlannerContainer();
    }

    const plannerUrl = document
      .getElementById("datePickerMenu")
      .getAttribute("plannerurl");
    const date = await this.getDateInCorrectFormat(true, addend);
    const data = await this.fetchPlannerData(date, plannerUrl.split("/")[4]);

    // Clear existing content
    this.plannerContainer.innerHTML = "";

    // Create and add title
    const dateText = `${await this.getDateInCorrectFormat(false, addend)}`
      .split(" ")
      .slice(0, 4)
      .join(" ");
    ...
    if (!data || data.length === 0) {
      ...
      timeSlots.forEach((slot) => {
        ...
        slot.elements.forEach((element, index) => {
          const plannerElement = this.createPlannerElement(
            element,
            index,
            numElements,
            elementWidthPercentage,
